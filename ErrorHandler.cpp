...
#define DEVLOG_FILENAME "logs/DevLog.txt"
...
namespace err {
	...
	void Logger::SetDevlogFile(const char* const filename) {
		Get().devlog_filename = filename;
	}
...
#ifdef _DEBUG
		devlog_output_file.open(devlog_filename, std::fstream::out | std::fstream::app);
		if (!devlog_output_file) {
			if (m_console) {
				std::cout << "[-] Can't open file: \"" << devlog_filename << "\"\n";
			}
			if (brief_output_file) {
				brief_output_file << "[-] Can't open file: \"" << devlog_filename << "\"\n";
			}
			if (extended_output_file) {
				extended_output_file << "[-] Can't open file: \"" << devlog_filename << "\"\n";
			}
		}
#endif
	}
...
#ifdef _DEBUG
		devlog_output_file << curr_time << "[" << symbol << "] " << message << '\n';
#endif
		CloseLogFiles();
	}
...
#ifdef _DEBUG
		m_console = true;
		devlog_filename = DEVLOG_FILENAME;
#else
		m_console = false;
#endif
		m_messages = 0;
...
#ifdef _DEBUG
		// devlog
		devlog_output_file << "TOTAL MESSAGES: " << m_messages << '\n';
		devlog_output_file << "TOTAL WARNINGS: " << m_warnings << '\n';
		devlog_output_file << "TOTAL ERRORS  : " << m_errors << "\n\n";
#endif
		// extended
